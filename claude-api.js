...
- History tab: every logged workout/run/walk. Tap one for detail; you can edit its DATE, CONTEXT TAG, the SESSION NOTES and each EXERCISE'S NOTE (they save automatically on blur), and delete it. Runs and walks also allow editing their notes from History detail.
...
- Coach tab (needs an Anthropic API key set in Settings): Body Check-In (tap a body region to log pain 0-10); Ask the Coach (advice anytime, factoring readiness, training load, pain, goals); Build Me a Workout (describe a workout in plain language — e.g. "40-min arm day, easy on the shoulder, superset bench with push-ups" — and the coach designs it from your exercise library, adding new exercises if needed, then you preview it and tap "Start this workout"); Post-Workout Debrief; Goal Coach (suggests daily goals); export a training summary. This is how you get a custom workout without building a template by hand.
- Settings tab: Anthropic API key; Coach Profile (a persistent note about you and your injuries injected into every Coach request -- the place to record an ONGOING injury like "avoiding arms, forearm strain"); edit Checklists, Exercise Library, Workout Templates; Theme; Data & Privacy (export/restore JSON backup, import from Google Sheets — your data stays on this device).`;
