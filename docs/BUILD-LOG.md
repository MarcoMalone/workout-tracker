...
| `(supersets)` | **Supersets — round-interleaved (Stage 1).** Template editor reworked from a flat checkbox list into an ordered per-exercise builder (reorder, per-exercise sets × reps/seconds, add/remove) with a **⛓ link toggle** that supersets a row with the one above — linked rows visibly indent under a lime bar with a "superset with above" tag. On save, `assignSupersetIds` derives an optional `supersetId` per row (no DB migration). Logging groups consecutive same-`supersetId` exercises into a **`.superset-block`** rendered in **rounds** (round r = each exercise's r-th working set + trailing drops), with **group-aware rest** (rest fires only after the group's last exercise; earlier ones show a "Next: X" cue), a per-exercise-per-round **"+ drop"**, and **"+ Add round"**. `appendSetRow` reused via an `opts` hook (label / onCheck / reRender). Also: edit exercises from the library, unilateral "per side" labels, capitalized Body-Part dropdowns. New `docs/ROADMAP.md` logs deferred Coach ideas (Coach-builds-a-workout, rename framing). SW v45→v46. Spec at `docs/superpowers/specs/2026-07-08-supersets-design.md`. |
| `f405e05` | **Live-feedback fixes (during Marco's first real logging session).** Strip empty sets on finish + filter them from the "Previous:" line (kills "null × null" from stray/skipped sets); drop set inserts after the last *completed* set (not only at the end); "Auto rest timer" toggle to disable the countdown; collapsible About; reload returns to the last tab (persisted `lastTab`) with the resume bar intact. SW v44. |
...
- Coverage grew **39 → 139 tests**. Pure logic is unit-tested (e1RM, ACWR, weekly
